...
const PhotoCard = ({ c }: { c: Card }) => (
  <div className="relative shrink-0 w-[180px] h-[180px] md:w-[200px] md:h-[200px] rounded-lg overflow-hidden shadow-elevated border border-border">
    ...
    <div className="absolute inset-x-0 bottom-0 p-2.5">
      ...
      <div className="text-white/75 text-[10px] mt-0.5">{c.company}</div>
    </div>
  </div>
);
  ...
  return (
    <div className="group relative overflow-hidden">
      <div
        ...
        style={{ animation: `mep-scroll-${direction} 40s linear infinite` }}
      >
        {loop.map((c, i) => (
    ...
    <section className="relative bg-background border-t border-border overflow-hidden">
      <div className="container max-w-[1680px] mx-auto px-6 py-20 md:py-28">
        <div className="max-w-3xl mb-12 md:mb-16">
          <div className="flex flex-wrap gap-x-6 gap-y-1 items-baseline">
            {["Mechanical", "Electrical", "Plumbing"].map((word) => (
              ...
              >
                <span className="text-[hsl(var(--accent))] font-black">{word[0]}</span>
        ...
        <div className="space-y-3">
          <ScrollRow cards={rowTop} direction="left" />
          <ScrollRow cards={rowBottom} direction="right" />
        </div>
      </div>
